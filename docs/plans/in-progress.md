...
- **TESTING as a refactor/bugfix safety net: BIMODAL (A inside `src/game/`, C+ outside)** — ~587 vitest cases blanket the pure domain (`damage`, `stats/compute`, `generator`, `drops`, `scaling`, `levels`, …), so a bug or refactor in `src/game/` has a real net. But the React hooks (12 of 16 untested, including the `useCombatLoop` state machine), the Convex mutations (`recordKill` / `usePotion` / `syncHp` — server-authoritative, **zero tests**), and the big world components (zero tests) are blind spots. The mutations are hard to unit-test directly because every one opens with `authComponent.getAuthUser(ctx)` (better-auth component read — `convex-test`'s `withIdentity()` doesn't satisfy it). The unlock pattern is **extraction**: pull the deterministic decision logic into a pure `src/game/` function and unit-test that, leaving the mutation a thin auth+IO wrapper. Started for `recordKill` — its kill classification and boss-kill tally now live in `src/game/combat/kill.ts` (`classifyKill` / `tallyBossKill`, 14 tests). The same treatment is still owed to the camp-tier/zone-completion branch, `usePotion`, `syncHp`, and `respawnDead`. Continuing this is the single highest-leverage ergonomics move.
