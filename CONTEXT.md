...
- **Swapping hands is supported.** When both hands are filled with a legal dual-wield pair (1H + 1H, same archetype), the player can swap which weapon occupies the main vs off-hand from inside the inventory modal — by dragging one onto the other, or via the "Inverter mãos" button below the equipment grid. The swap matters because the main hand swings first on a fresh combat (see "Tick order" under Combat Resolution) and the swinging weapon's local mods source that tick's damage. Allowed during combat, no cooldown (mirrors the general equip-swap rule).
