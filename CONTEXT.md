...
**Staff base damage = 2.0 × wand base damage at every tier** (T1..T21). On the attack side, 2H weapons follow a `~2.4× 1H` rule (greatsword 2.4× sword, twoHandedAxe 2.4× axe). The +0.4× over the original 2.0× design intent restores the 2H damage advantage that `physicalDamageFlat` (max +100 to both min and max) erodes at endgame — adding 100 flat to a small 1H base is a larger relative boost than adding it to a 2H base, which compressed the in-practice ratio from 2.0× toward parity with dual-wield. AS stays slow on 2H weapons (greatsword 1.1, twoHandedAxe 1.05) — the archetype is heavy slow hits with high damage per swing, not a faster swing rate. Other staff stats are independent: attack speed 1.2, crit 6.0, implicit `+% Spell Damage` 75-125% — they don't scale by the 2× rule. The doubling compounds through the spell damage implicit's multiplicative scaling, so the staff edge widens at high tiers but stays bounded.
